            ...
            <div>
              ...
              {servicesOpen && (
                <div className="mt-4 ml-4 space-y-3">
                  {servicesItems.map((item) => (
                    <Link key={item} href={`/${item.toLowerCase().replace(/\s+/g, "-").replace("&", "and")}`} onClick={() => setIsMobileMenuOpen(false)} className="block text-gray-300 hover:text-cyan-400">{item}</Link>
                  ))}
                </div>
              )}
            </div>
